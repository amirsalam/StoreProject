...
export default function Terms() {
    ...
    const sections = tList<LegalSection[]>('legal.terms.sections') ?? [];

    return (
        <StorefrontLayout>
            <Head title={`${t('legal.terms.meta_title')} — ${brandTitle}`}>
                {/*
                  The content is an unreviewed outline, not a policy. Keep it
                  out of search results until real terms replace it; drop this
                  tag in the same change that publishes the real wording.
                */}
                ...
                <Container>
                    <div className="mx-auto max-w-3xl">
                        <header className="flex flex-col gap-3">
                            ...
                            <h1 className="font-display text-4xl font-semibold tracking-tight text-balance sm:text-5xl">{t('legal.terms.title')}</h1>
                            <p className="text-muted-foreground text-pretty sm:text-lg">{t('legal.terms.lead')}</p>
                        </header>
                        ...
                        <div className="border-border/60 mt-16 flex flex-col items-start gap-3 border-t pt-8">
                            <h2 className="font-display text-xl font-semibold tracking-tight">{t('legal.terms.contact.title')}</h2>
                            <p className="text-muted-foreground text-sm">{t('legal.terms.contact.body')}</p>
                            <Button asChild variant="outline">
                                <Link href={route('contact')}>{t('legal.terms.contact.action')}</Link>
                            </Button>
                        </div>
                    </div>
                </Container>
