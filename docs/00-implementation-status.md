...
- **Completed:** `/api/v1` (user/dashboard/notifications), Sanctum tokens (`ApiTokenController`, scopes, shown-once, audited), `personal_access_tokens`. **Public license API** (unauthenticated, key-as-credential): `POST /api/v1/licenses/activate` · `POST …/deactivate` · `GET …/validate` → `LicenseController` + `LicenseActivationService` + `LicenseActivationRequest`; tenant resolved by host (`ResolveTenant` on the route group), `license-api` limiter (60/min/IP), uniform 404 for unknown/foreign/wrong-product keys. Tests: `LicenseActivationApiTest` (16).
- **Public API reference** at `/api-reference` (`Resources\ApiReferenceController`): endpoints listed by route name with method/path read from the router, license error codes and example payload rendered from the live exception/controller code; `ResourcePagesTest` fails if an `api.v1.*` route is undocumented.
- **Missing:** gateway middleware stack, OAuth2/OIDC, outbound webhooks, OpenAPI/SDKs, the portal, metering/monetization. **Token abilities (read/write/admin) are stored but not enforced on any route** — every valid token can call every account endpoint.
...
| 2026-09-22 | **Customers page shipped** (+1 page / +1 test file → 36). Unlike about/blog this link was not a 404: footer *and* top-nav "Customers" pointed at `/#testimonials`, an anchor into the home page — verified working (lands exactly on the section), but a Company-column link reading "Customers" deserves its own page. Added `Route::inertia('customers', 'customers')->name('customers')` + `pages/customers.tsx` on `StorefrontLayout` (hero, three use-case cards, the testimonial grid — reusing the existing `testimonials.*` keys verbatim rather than inventing new customer claims — and a browse/about CTA), `customers.*` i18n ×4, and repointed all three "Customers" links (desktop nav, mobile nav, footer) at `route('customers')`. The home page keeps its `#testimonials` section, so old anchor links still resolve. `Marketplace\CustomersPageTest` (3): guest render + testimonial copy present, Arabic locale/RTL, en/fr/es/ar key parity (no-fallback lookup). Verified: full suite green (257 tests, 1054 assertions); `npm run build` green; Pint/ESLint/Prettier/tsc clean; browser — all 3 links resolve to `/customers`, 3 quotes render, fr + ar/RTL, no console errors, no overflow at 375px. Note: the testimonial quotes/names are the repo's existing placeholder marketing copy, not real customers. Remaining `#` placeholders: contact, legal (terms/privacy/license/refunds), docs, guides, api, status, changelog, roadmap. |
| 2026-09-22 | **Contact form + admin inbox shipped** (+2 pages / +2 test files → 38; +1 table / +1 model / +1 mailable). The footer "Contact" link (`تواصل معنا`) was a `#` placeholder. Public `/contact` (`ContactController@show/store`) posts to a rate-limited route (`throttle:5,1`) behind `StoreContactMessageRequest`: validation + a `prohibited` honeypot field (`website`, `sr-only`), sender linked to their account when logged in. **Messages are stored first** in a new tenant-scoped `contact_messages` table (`ContactMessage` + factory, `BelongsToTenant`, new/read/archived + `read_at`), so the feature works with **no mail configuration**; an email copy goes out only when `CONTACT_NOTIFY_TO` is set (`config/contact.php` → `ContactMessageReceived` mailable, reply-to = sender, markdown view), and a mailer failure is logged without losing the message or failing the request. Admin inbox at `/admin/contact` (list + search/status filter, unread count, mark read / archive / delete, mailto reply) + "Contact" sidebar entry. `contact.*` i18n ×4 including the server-side success flash. `Contact\ContactFormTest` (10: render, guest + logged-in submit, validation, honeypot, rate limit, mail on/off, mailer-failure durability, locale parity) and `Admin\ContactInboxTest` (8). Also registered `ContactMessage` in `TenantIsolationTest::tenantedModels()` — the repo's reflection guard caught the omission. Verified: full suite green (277 tests, 1148 assertions); `npm run build` green; Pint/ESLint/Prettier/tsc clean; dev DB migrated; browser — real submission stored + localized success flash, honeypot confirmed non-visible (1×1 clipped), fr + ar, no console errors, no overflow at 375px (test row deleted afterwards). Remaining `#` placeholders: legal (terms/privacy/license/refunds), docs, guides, api, status, changelog, roadmap. |
| 2026-09-23 | **Terms page shipped as a marked draft** (+1 page / +1 test file → 39). The footer "Terms" link (`الشروط`) was a `#` placeholder. Per the user's choice, this is a **structural outline, not a policy**: `Route::inertia('terms', 'legal/terms')->name('terms')` + `pages/legal/terms.tsx` renders 13 numbered sections (acceptance, operator, accounts, purchases, licenses, refunds, vendors, acceptable use, IP, liability, termination, governing law, changes) whose bodies are explicit *drafting notes* ("state…", "cover…") rather than pseudo-legal prose, under a loud amber "Unreviewed draft — not binding" banner, plus a "Draft · not yet published" status label, an anchor table of contents, and a contact CTA. The page sends `<meta name="robots" content="noindex">` so an unreviewed legal page cannot be indexed — **remove that tag in the same change that publishes real wording**. New `legal.*` i18n block ×4 (banner, status, TOC, 13 sections). `Marketplace\TermsPageTest` (4): render + section count, draft-banner presence (guards against shipping placeholder text without the warning), every section has id/title/body in every locale, and section ids identical across locales. Verified: full suite green (281 tests, 1497 assertions); `npm run build` green; Pint/ESLint/Prettier/tsc clean; browser — 13 sections + 13 TOC anchors, anchor jump lands correctly, noindex present in fr and ar/RTL, no console errors, no overflow at 375px. **No legal text was authored**; the wording must be written or reviewed by a lawyer before launch. Remaining `#` placeholders: privacy, license, refunds, docs, guides, api, status, changelog, roadmap. |
| 2026-09-24 | **Privacy page shipped as a marked draft; legal pages refactored** (+2 pages / test file renamed → 39). The footer "Privacy" link (`الخصوصية`) was a `#` placeholder. Same marked-draft contract as terms (loud "Unreviewed draft — not binding" banner, "Draft · not yet published" label, `robots: noindex`, anchor TOC, contact CTA), with 16 sections. **Sections that describe data flows are grounded in the code, not invented**: account fields (name/email/hashed password), order + billing fields, contact-form storage, IP/user-agent held against sessions and `activity_logs`, social-login payload, the session + `app_locale` cookies, and the existing protections (hashed passwords, encrypted gateway credentials, per-tenant scoping). Sections that require business or legal decisions (controller identity, purposes/lawful bases, processors, retention, rights, transfers, children, changes) stay explicit drafting notes. Extracted `pages/legal/legal-document.tsx` as the shared shell; `legal/terms.tsx` + `legal/privacy.tsx` are now thin wrappers, so both documents share one banner/TOC/noindex implementation. `TermsPageTest` → `Marketplace\LegalPagesTest`, now data-provider driven over both documents (10 tests): render + section count, draft-banner presence, **a source assertion that `robots: noindex` is still emitted**, per-locale id/title/body completeness, and id parity across locales — future legal docs join by adding one provider row. Verified: full suite green (287 tests, 1920 assertions); `npm run build` green; Pint/ESLint/Prettier/tsc clean; browser — privacy 16 sections + 16 TOC anchors, terms still 13 after the refactor, banner + noindex in fr and ar/RTL, no console errors, no overflow at 375px. **No legal text was authored**; wording needs a lawyer before launch, and the data-flow claims must be re-checked against the final build. Remaining `#` placeholders: license, refunds, docs, guides, api, status, changelog, roadmap. |
| 2026-09-24 | **License page shipped as a marked draft** (+1 page; `LegalPagesTest` gains a provider row → 15 tests). The footer "License" link (`الترخيص`) was a `#` placeholder. Same shared `LegalDocument` shell (banner, status label, `robots: noindex`, TOC, contact CTA), 12 sections on buyer license terms. Code-grounded sections state only what is **stored**: vendor-chosen free-text `license_type` labels shown on product pages; license/API-access purchases issuing a unique key, a per-product activation limit and an active/expired/revoked status; download grants with an optional per-product max; revocation timestamps and expiry dates. **Activation is deliberately not described as enforced**: no route increments `licenses.activations_count` (it is only initialised to 0 in `FulfillOrder`) and there is no activation endpoint — the "Activations" section is a drafting note that says to confirm enforcement before promising it. Note the home page `product_types.items.licenses.desc` copy already claims "Activate from your script with one request", which the code does not yet support (flagged separately). Verified: full suite green (292 tests, 2246 assertions); `npm run build` green; Pint/ESLint/Prettier/tsc clean; browser — 12 sections + 12 TOC anchors, banner + noindex, fr and ar/RTL, footer link resolves, no console errors, no overflow at 375px. **No legal text was authored.** Remaining `#` placeholders: refunds, docs, guides, api, status, changelog, roadmap. |
| 2026-09-24 | **License activation API shipped** — closes the "Activate from your script with one request" claim on the storefront (`product_types.items.licenses.desc`), which had no backing endpoint (`activations_count` was never incremented). `POST /api/v1/licenses/activate` + `/deactivate`, `GET /validate`, each taking `license_key` + `domain` (normalised: scheme/path/case/trailing dot stripped; may be an opaque instance id) + optional `product_id`. `LicenseActivationService` enforces key exists in the host's tenant, not revoked, not expired (status *or* `expires_at`), slots < `activation_limit`; activation is idempotent per domain; deactivate frees the slot and works on any status. `activated_domains` is the source of truth and `activations_count` is rewritten from it; activate/deactivate run in a transaction with `lockForUpdate` so concurrent calls can't overshoot the limit. Enumeration hardening: unknown, malformed, other-tenant, wrong-product and no-tenant (central host) lookups return the byte-identical `404 invalid_license`; keys are upper-cased and compared with `hash_equals` (guards MySQL's case-insensitive collation; a miss costs the same compare). Rate limit `license-api` = 60/min per IP. `bootstrap/app.php` now renders exceptions as JSON for `api/*` even without `Accept: application/json` (also affects the Sanctum routes: 401/422 are JSON, not a login redirect). Activations/deactivations logged to `ActivityLog`. `License` gains `isRevoked()`/`isExpired()`/`activatedDomains()`/`hasActivation()`; `canActivate()` now counts domains. **Fulfillment fix:** `FulfillOrder` now copies `tenant_id` from the order onto the issued `License`/`Download` — the Stripe-webhook path runs with no tenant in context, so card-paid licenses were stored with `tenant_id = null` (or the dev fallback tenant) and could never resolve on their store's host. +16 tests → **250 green**. No completion % moved. |
| 2026-09-25 | **Footer Resources column shipped; no placeholder links left** (+4 pages / +1 test file). Docs, Guides, API and Status were `#` placeholders (Changelog/Roadmap too). **Status** (`/status`, `SystemStatus` + `StatusController`): live database query, cache round-trip and queue-backlog check (a pending job older than 10 min = no worker), cached 60 s; failures are logged, never exposed. The footer's hard-coded "All systems operational" line now reads that snapshot (shared `system_status` prop) and links to `/status`. **API reference** (`/api-reference`): built from the router + `LicenseActivationException` + `LicenseController::payload()` (made public static for the example), per-endpoint curl examples generated from the definitions, license rate limit read from `LicenseActivationService::RATE_LIMIT_PER_MINUTE` (now shared with the limiter). **Docs / Guides** (`/docs`, `/guides`): marked draft outlines ("what the code does today" + drafting notes) on a new shared `components/draft-document.tsx` shell, which the legal pages now also use. Header "Docs" nav → `/docs` (was `/#faq`). Changelog/Roadmap links removed until there is content. All text ×4 locales. `ResourcePagesTest` (12): draft banner/noindex/locale completeness for docs+guides, every `api.v1.*` route documented, reference text in every locale, status operational + stalled-queue degrades page and footer, no `href: '#'` in the layout. Also on this branch: merged refunds (#4), sidebar RTL/hide and license API branches, and re-applied the License page *Activations* drafting note. **326 tests green.** Verified live on localhost:8000 (MySQL): all 14 footer/header targets 200, status checks pass, documented curl calls return the documented 404/422 JSON, Arabic RTL renders with LTR code blocks. |
| 2026-09-26 | **Card checkout wired end to end** (+1 class / +1 test file, 10 cases → **336 green**). Reported as checkout's "payments unavailable" notice. Two real gaps behind it: (1) **Admin → Payment Gateways was not read by anything** — checkout, PaymentIntent creation and the webhook only read `.env`. New `Domain\Payments\StripeCredentials`: the store's (tenant's) active Stripe gateway with both keys, else `.env`; keys always from one source; no tenant in context → `.env` only. `CheckoutController` (publishable key), `StripeGateway::createPaymentIntent` (secret) and `StripeWebhookController` (signature checked against the gateway's and the platform's webhook secret) use it; SaaS plan billing keeps the platform `.env` keys by design. (2) **Paid orders were never marked paid**: `/webhooks/stripe` sent every event to the billing processor, which ignores `payment_intent.*`/`charge.*`, and nothing dispatched `ProcessPaymentWebhook` — so card orders stayed `pending` and were never fulfilled. The controller now routes `OrderPaymentProcessor::HANDLED_EVENTS` to the queued job (a worker must run) and everything else to billing — exactly one processor per event, since both write the unique `webhook_events` gate. Tests post genuinely signed events; both routing tests fail against the old controller. Still needed to take a payment locally: Stripe test keys in the admin gateway (or `.env`), `stripe listen --forward-to localhost:8000/webhooks/stripe` for the webhook secret, and a queue worker. |
| 2026-09-26 | **Stripe for checkout is configured only in the dashboard** (per the user: "config payment gateway in dashboard, not .env"; +4 tests → **340 green**). `StripeCredentials` no longer falls back to `STRIPE_KEY`/`STRIPE_SECRET` — without an active Stripe gateway with both keys, checkout shows "payments unavailable". **Test connection** now really checks Stripe: key prefixes, test/live consistency, keys vs the gateway's environment, then a read-only `balance` call via `StripeGateway::verifySecretKey()` (faked in tests) — previously it only checked fields were non-empty. The gateway form shows the store's webhook URL (copy button) and the exact events to subscribe (`OrderPaymentProcessor::HANDLED_EVENTS`). Still in `.env` by design: SaaS **plan billing** (`STRIPE_SECRET` + `STRIPE_PRICE_*`, platform account) and its webhook secret. |
| 2026-09-26 | **Payment gateway configurable from Workspace → Billing** (+8 tests → **348 green**). The user wanted to set up Stripe from the billing page in the dashboard. New *Payment gateway* card on `workspace/billing`: mode (test/live), publishable key, secret key, webhook signing secret, active toggle, Save + Test connection, and the store's webhook URL + events (shared `components/stripe-webhook-steps.tsx`, also used by the admin form). `Workspace\PaymentGatewayController` (`PUT workspace/billing/gateway`, `POST …/gateway/test`) + `UpdatePaymentGatewayRequest` (key prefixes validated; keys required only on first save; blank secret = keep) save the workspace's single Stripe `PaymentGateway` through `PaymentGatewayService` (encrypted, audited) — the record checkout already reads. Access: `Tenant::canManagePayments()` = owner, workspace admin or platform admin; members get neither the form nor any gateway data (403 on the endpoints). The page receives only `has_secret_key`/`has_webhook_secret` flags, never secret values. The old "set STRIPE_SECRET in .env" banner now says plan upgrades (platform billing) aren't connected, distinct from the store's gateway. |
| 2026-09-26 | **Fix: admin product delete crashed for sold products** (+2 tests → **350 green**). `order_items`/`licenses`/`downloads`/`subscriptions` reference `products` with `restrictOnDelete`, so deleting any product that had been sold threw a 1451 FK violation (500; the UI showed nothing). `Admin\ProductController::destroy` now archives a product with sales history (`Product::hasSalesHistory()`, checked across tenants like the FK) — hidden from the store, buyers keep their access — and says so; unsold products are still deleted. Dropped `preserveScroll` on delete so the result message at the top of the list is visible. |
